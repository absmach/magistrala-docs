...
<Callout type="info" title="Rewritten for Atom's GraphQL API">
  The **Workspaces**, **Devices**, **Channels**, **Groups**, and now **Users** sections below are
  current — all five were rewritten against Atom's actual GraphQL schema
  (`src/graphql/{tenants,entities,resources,groups,auth,credentials}.rs` in the Atom repository)
  and cross-checked against what the CLI and UI actually send over the wire
  (`cli/atom_commands.go`, `pkg/atom/api.go`), not invented from the schema alone. **Messages** is
  a different story again: publishing over HTTP/MQTT/CoAP/WS is unrelated to Atom and still works
  as described, aside from the `domain_id` → `workspace_id` path segment rename (already fixed).
</Callout>
...
> - `email` is normalized (lowercased) and used as the login identifier — it is not a field on the `Entity` type itself, and no mutation for changing it was found in this pass; treat email changes as unverified until confirmed against a maintainer.
...
<Callout type="warn" title="Not re-verified in this pass">
  A few of the old REST API's user operations have no confirmed GraphQL equivalent found here:
  changing a user's **email** or a distinct **username** (Atom's login `identifier` is the email
  itself, normalized — there's no separate username field or credential kind for one), and any
  bulk/batch user operations (consistent with [Devices](#create-device): no `createEntities`
  exists). Treat these as open questions for a maintainer rather than assuming they're unsupported.
</Callout>
