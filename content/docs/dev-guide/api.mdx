...
<Callout type="warn" title="Users, Domains, Clients, Channels, and Groups sections below are stale — the REST surface they describe no longer exists">
  Verified against `cmd/` and `docker-compose.yaml`: there are no standalone users, clients,
  channels, domains, or groups Magistrala services anymore, and no `transport.go` for any of them
  — see [Entities](/dev-guide/entities) and [Architecture](/dev-guide/architecture). Those
  concepts (now Workspaces/Devices/Channels/Groups) are managed through **Atom's GraphQL API**
  (`src/graphql/{query,mutation}.rs` in the Atom repository), which the CLI wraps for the small
  set of operations it exposes — see the [CLI reference](/dev-guide/cli/introduction-to-cli) for
  what's actually runnable today. Rebuilding this section from Atom's GraphQL schema is a
  substantial follow-up on its own, out of scope for this pass — do not trust specific endpoints,
  request/response shapes, or status codes below for those five sections without re-verifying
  against the GraphQL schema first. The **Messages** section further down is a different story:
  publishing over HTTP/MQTT/CoAP/WS is unrelated to Atom and still works as described, aside from
  the `domain_id` → `workspace_id` path segment rename (fixed below).
</Callout>
