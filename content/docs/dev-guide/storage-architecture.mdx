...
At the core of Magistrala are a set of foundational domain entities that define how messaging, identity, and access control are structured across the platform:

- `Users` — humans, services, and workloads that authenticate and manage resources
- `Clients` — represent devices or applications
- `Channels` — define topics for message routing
- `Groups` — organize entities hierarchically
- `Domains` — tenant boundaries that scope everything above

In current Magistrala, this identity and metadata layer is managed by **[Atom](https://github.com/absmach/atom)**, a lightweight identity and authorization service (a slimmer alternative to Keycloak). Atom persists all of this state in **PostgreSQL**, which acts as the system of record for the control plane. A Magistrala **domain** maps directly to an Atom **tenant**, so multi-tenancy is a first-class property of the store.

The data defines not just what exists in the platform, but how it relates, who can access it, and how messages are routed. Unlike the high-frequency message payloads that flow through the system, this metadata changes relatively infrequently. But when it does, it must be updated atomically, consistently, and reliably, because it forms the foundation of the entire system.
...
- **Enforced Relational Integrity**
  The schema captures and enforces real-world constraints — such as "a client must belong to a valid domain" or "a channel cannot be deleted while it has active connections." These relationships are protected at the database level using foreign keys, cascading rules, and check constraints.
...
Imagine a tenant issues an API call to connect a `client` to a `channel`. This operation touches multiple critical entities: the client, the channel, and the domain (tenant) context that governs them.

In a relational store, this relationship is enforced through a normalized schema. For example:

- A connection references both the client and the channel through foreign keys, ensuring each exists and belongs to the correct domain.
- Constraints guarantee that a client can only be connected to a channel **within the same domain**, so cross-tenant linkage is impossible.
- A uniqueness constraint prevents duplicate connections of the same type.

If any part of the linkage is invalid — a disabled client, a mismatched domain, or a malformed connection type — PostgreSQL **rejects the operation entirely**. The constraints are enforced at the database layer, regardless of application logic, guaranteeing internal consistency.
...
- Documents with an invalid `client_id`, a mismatched `domain_id`, or an undefined connection type can be stored without error.
- No built-in constraint prevents cross-domain linking or ensures the client exists.
...
In PostgreSQL you can rely on foreign keys and relational joins to ensure any connected client is valid, and then use an efficient, indexed query:
...
- **Operational simplicity**: the database should automate tasks like partitioning and retention, reducing the need for dedicated administration.
...
Purpose-built time-series databases such as InfluxDB are incredibly fast at ingestion and excellent for simple metrics like CPU usage or raw sensor values. However, their query languages and data models are optimized primarily for time-based lookups, not the rich, multi-attribute queries required in real-world IoT deployments — and high-cardinality workloads (millions of devices, each with a unique tag set) are a well-documented pain point for them.

Magistrala routinely handles hundreds of thousands to millions of concurrently publishing devices — industrial chillers, smart meters, air-quality sensors — each pushing telemetry with unique tags, locations, and context. In benchmarks published by TigerData, TimescaleDB sustains roughly **3.5x better insert throughput** than InfluxDB at high cardinality, and answers real-world, multi-dimensional queries **several times to orders of magnitude faster**, while remaining 100% Postgres-compatible. That compatibility is decisive: Magistrala keeps the reliability, tooling, and full SQL of PostgreSQL while gaining time-series scale.
...
Behind the scenes, TimescaleDB's hybrid row-columnar storage engine automatically stores data in the format that makes the most sense for its lifecycle:

- Recent data lives in the **rowstore** (fast inserts).
- Older data is compressed into the **columnstore** (fast analytics).

By default each chunk covers 7 days, and you can tune this to suit your workload. To keep performance high when scaling, choose a strategic chunk size and limit the number of hypertables per service. If chunks are too big, the benefit of partitioning diminishes because a partition holds more data than the PostgreSQL cache can manage; if there are too many chunks, you can overwhelm the query planner and add management overhead.
...
- **Sustained high-ingest performance**: new data is written only to the most recent chunk. Because that chunk is small, its indexes fit comfortably in memory. This avoids the disk-I/O bottleneck that cripples a large single-table setup, allowing Magistrala to maintain a consistently high ingest rate of over 100,000 rows/second even as the total dataset grows to billions of rows.
- **Massively faster queries**: when a user queries for data within a specific time range, the planner uses **chunk pruning** — it instantly knows it only needs to scan the relevant chunks and ignores the vast majority of the data on disk. In some benchmarks this makes queries orders of magnitude faster than the equivalent on a standard PostgreSQL table.
...
**"Can `user1` publish to `channel1` in tenant `d1`?"**

The calling service makes a single, low-latency check against Atom:

```text
authz.check(subject=user:user1, action=publish, object=channel:channel1, tenant=d1)
```

Atom evaluates the subject's role assignments and direct policies, resolves the matching permission blocks within the tenant scope, applies any ABAC conditions, and returns **allow** or **deny**. Because this is a purpose-built engine rather than a set of recursive SQL joins, authorization stays fast and predictable even as the number of entities, roles, and policies grows — which is critical for a real-time messaging system that authorizes on every publish and subscribe.
