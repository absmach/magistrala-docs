...
1. **Connection & authentication (Atom):**
   The HVAC sensor connects to a Magistrala protocol adapter and presents its credentials (an API key or a certificate). The adapter validates those credentials against **Atom**, which confirms the client is a known, active entity and establishes its identity.

2. **Authorization to publish (Atom via FluxMQ):**
   Before accepting any data, the broker's auth bridge asks Atom whether the sensor may publish to the target channel — a `Check` for the `publish` action on that channel within the client's domain. Atom evaluates the relevant roles and policies and returns **allow**.
