...
- **Identity & core metadata**: users, devices, groups, channels, workspaces, and the roles and policies that govern them. This data changes relatively infrequently but must always be consistent.
- **Time-series message data**: the billions of messages published by devices, where each message is an immutable event stamped with a precise time.
...
Magistrala keeps its familiar product concepts — workspaces, users, devices, channels, groups, connections, rules, reports, and alarms — but it does **not** run its own identity, credential, role, or policy database. Instead it builds directly on **[Atom](https://github.com/absmach/atom)**, a lightweight identity-and-authorization service (a slimmer alternative to Keycloak) that persists everything in **PostgreSQL** and acts as the system of record for the control plane.
...
| ------------------------------------- | ---------------------------------- | ------------------------------------------------- |
| Workspace                             | Tenant                             | `tenants` row (tenant id = workspace UUID)        |
| User                                  | Entity (`kind = human`)            | `entities` row, usually global (`tenant_id` null) |
| Device / Gateway (device or service)  | Entity (`kind = device`/`service`) | tenant-owned `entities` row                       |
...
- **ACID Transactions**
  PostgreSQL guarantees atomicity, consistency, isolation, and durability. Multi-step operations — like creating a user, assigning devices, or updating channel access — either succeed entirely or not at all. This eliminates any chance of partial, invalid state.

- **Enforced Relational Integrity**
  Atom's schema captures real-world constraints at the database level. An entity's `tenant_id` is a foreign key into `tenants` (`ON DELETE CASCADE`), so a device can never belong to a non-existent workspace; a `credential` references its `entity_id` the same way, so credentials can't outlive their owner; a `CHECK` limits an entity's `kind` to `human`/`device`/`service`/`workload`/`application`; and a unique index on `(name, tenant_id)` keeps names unambiguous within a tenant. These rules hold regardless of application logic.
...
### Modeling a Device-to-Channel Connection

Connecting a `device` to a `channel` shows both why the relational model matters and how Atom avoids bespoke join tables. There is **no `connections` table** — a connection is expressed with Atom's generic access primitives:

- **Preferred — a role assignment.** A role carries a permission block that applies to the channel resource with the `publish`/`subscribe` actions, and that role is assigned to the device entity (or to a Principal Group it belongs to).
- **Strict runtime links — a Direct Policy.** A trusted Magistrala service can attach a Direct Policy (`subject = device entity`, `permission_block = publish/subscribe on the channel resource`). Direct Policies are created only by trusted flows and are audited.

Both the device (an `entities` row) and the channel (a `resources` row) carry a `tenant_id` foreign key, so a device can only ever be linked to a channel **within the same tenant** — cross-tenant linkage is structurally impossible, and there is no separate `connections` or `direct_grants` table to keep in sync.
...
The relational model also makes listings fast and safe. Suppose you need every active device in a workspace tagged `region:EU`. Because Atom indexes the `attributes` JSONB (a GIN index) alongside `tenant_id` and `kind`, this is a single indexed query:
...
In Magistrala, every piece of device data, event log, or telemetry update is recorded as a message with a timestamp. This makes message storage a time-series challenge. Messages are generated, queued, and consumed in a stream and exhibit specific properties:
...
| -------------------- | ------------------------------------------------- | ------------------------------------------------ |
| **Tenant**           | Top-level boundary                                | Magistrala workspace `w1`                         |
...
| **Role**             | A named collection of Permission Blocks           | `Plant Operator` bundles device + channel access |
...
Authorization isn't only an API-layer concern. Magistrala's message broker, **FluxMQ**, ships with an **auth bridge** (`fluxmq-auth`) that authorizes every publish and subscribe. On each attempt the broker invokes its `auth_on_publish` / `auth_on_subscribe` hooks, which parse the topic into a workspace and channel and call Atom's `Check` (with the `publish` or `subscribe` action) before the message is accepted. A device cannot publish to a channel it has no permission for, because that check happens in the broker itself — identity, authorization, and transport are wired together end to end.
...
An HVAC sensor (a Magistrala device) in `Building-A` is configured to publish temperature readings to a channel. A facility manager later logs into a dashboard to view the temperature trends from the last 24 hours.

1. **Connection & authentication (Atom):**
   The HVAC sensor connects to a Magistrala protocol adapter and presents its credentials (an API key or a certificate). The adapter validates those credentials against **Atom**, which confirms the device is a known, active entity and establishes its identity.

2. **Authorization to publish (Atom via FluxMQ):**
   Before accepting any data, the broker's auth bridge asks Atom whether the sensor may publish to the target channel — a `Check` for the `publish` action on that channel within the device's workspace. Atom evaluates the relevant roles and policies and returns **allow**.
...
4. **Message persistence (TimescaleDB):**
   A rule matching that channel selects the reading for storage through its `save_senml` output and republishes it onto the broker's `writers` stream. The `timescale-writer` service, consuming that stream, normalizes the payload and inserts it as a new row in the `messages` hypertable within **TimescaleDB**. Storage is selective rather than automatic — see [Message Storage Backends](#message-storage-backends) for what feeds the writers.
...
Not every payload fits neatly into a relational row or a time-series point. For larger or binary assets, Magistrala uses **SeaweedFS**, an S3-compatible object store. Services reach it through standard S3 configuration (`MG_BACKEND_OBJECT_STORAGE_ENDPOINT`, plus bucket, region, and access key); the UI backend, for example, stores objects there via the S3 API. Keeping bulky objects out of PostgreSQL and TimescaleDB preserves the performance characteristics that make those stores good at their respective jobs, while still giving the platform durable, horizontally scalable storage for the data that belongs there.
...
- **Writers** consume a durable stream on the internal message bus (FluxMQ, with NATS available as a build-time alternative) and store messages in the chosen backend.
...
### What feeds the writers

The broker provisions two durable streams that matter for storage. Their bindings use different syntax in FluxMQ and NATS, but their roles are the same:

| Stream    | FluxMQ binding     | NATS subject | Receives                                                                      |
| --------- | ------------------ | ------------ | ----------------------------------------------------------------------------- |
| `m`       | `m/#`              | `m.>`        | Every message published to any channel, captured automatically by the binding |
| `writers` | `$queue/writers/#` | `writers.>`  | Only messages a service explicitly republishes there                          |

Writers consume **`writers`**, not `m`. Nothing publishes into `writers` on its own: Magistrala's built-in producer is the Rules Engine, whose `save_senml` and `save_json` outputs republish a matched message for storage. Persistence is therefore *selective* — a message is stored because a rule chose it, not because it was published.

<Callout type="info" title="Community Edition">
  Community Edition includes a Rules Engine build limited to one rule per deployment. That rule can
  use `save_senml` or `save_json` and feeds the same `writers` stream as Enterprise Edition. The
  Enterprise Edition build removes the rule-count limit. In either edition, writers can start,
  connect and report healthy while persisting nothing when no matching storage rule exists.
</Callout>

Pointing a writer directly at the `m` stream would make it store all channel traffic without a rule. It is **not supported today**: a writer's stream name and topic prefix are fixed when the service is built, and a message delivered out of `m` fails the writer's topic parsing, which expects the `writers/` prefix. Setting `topics` in the writer's `config.toml` does not achieve it either — that field filters *within* the stream the writer is already bound to, so it cannot move the writer to a different one.

A deployment that specifically needs to persist all channel traffic without a rule must provide a custom consumer for the broker's `m` stream or modify the writer's broker adapter, using `consumers/writers` as the reference implementation.
