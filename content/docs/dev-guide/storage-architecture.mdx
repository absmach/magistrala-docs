...
description: How SuperMQ uses PostgreSQL, TimescaleDB, and SpiceDB — a polyglot persistence strategy for scalable, secure, and performant IoT message storage.
keywords:
...
**“Can user alice, who belongs to group `eng_team`, which has editor rights on domain `smart_building`, publish to channel `hvac_updates` owned by client `sensor_12`?”**
