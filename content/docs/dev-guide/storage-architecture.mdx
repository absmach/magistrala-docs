...
Application-specific fields that Atom doesn't model natively live directly in the object's `attributes` JSONB column, so Magistrala never needs a second store. Because a Magistrala **workspace** is an Atom **tenant**, multi-tenancy is a first-class property of the schema — every entity, resource, group, role, and credential is scoped to a tenant.
...
- **Writers** consume a durable stream on the internal message bus (FluxMQ) and store messages in the chosen backend.
...
The broker provisions two durable streams that matter for storage:

| Stream    | FluxMQ binding     | Receives                                                                      |
| --------- | ------------------ | ------------------------------------------------------------------------------ |
| `m`       | `m/#`              | Every message published to any channel, captured automatically by the binding |
| `writers` | `$queue/writers/#` | Only messages a service explicitly republishes there                          |
