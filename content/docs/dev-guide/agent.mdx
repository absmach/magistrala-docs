...
<Callout type="warn">
This page's specifics (Docker Compose stack, `MG_AGENT_*` env vars, `make provision`/`scripts/provision.sh`, the port-9999 HTTP API, Node-RED integration) could not be verified against `/home/ian/work/magistrala` — there is no `agent` package, no `provision.sh`, and none of the `MG_AGENT_*` variables appear anywhere in that repo. If this describes a separate `github.com/absmach/agent` repo, it needs verifying against that repo directly before Latest ships it; if it doesn't correspond to anything currently shipped, it should be removed. Treat everything below as unconfirmed.
</Callout>
