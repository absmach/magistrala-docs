...
<Callout type="warn" title="This page describes a service that no longer exists standalone">
  There is no separate Clients service. Devices (Magistrala's current public term — see
  [Architecture § Core Concepts](/dev-guide/architecture#core-concepts)) are Atom entities of kind
  `device`, managed through Atom's GraphQL API — see the CLI's [Devices
