...
<Callout type="warn" title="This page describes a service that no longer exists standalone">
  Confirmed via `docker-compose.yaml` and `cmd/`: there is no separate Clients service. Devices
  (Magistrala's current public term — see [Architecture § Core
  Concepts](/dev-guide/architecture#core-concepts)) are Atom entities of kind `device`, managed
  through Atom's GraphQL API. For current, verified commands see the CLI's [Devices
  page](/dev-guide/cli/devices-cli) and [Gateways page](/dev-guide/cli/gateways-cli). The content
  below documents the pre-Atom architecture and is kept for historical reference only.
</Callout>
