...
<Callout type="warn" title="Atom-backed service">
  Clients are currently backed by Atom entities, with Magistrala compatibility code in
  `pkg/clients` and `pkg/atom`. The current Docker configuration maps `MG_CLIENTS_URL` to Atom's
  `/entities` endpoint, and there is no standalone `cmd/clients` entry point in the Magistrala
