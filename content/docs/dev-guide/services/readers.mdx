...
```bash
curl -X GET "http://localhost:8180/{workspaceID}/channels/{channelID}/messages?subtopic=temperature&from=1700000000&to=1700500000&limit=5" \
...
```bash
curl -X GET "http://localhost:9009/{workspaceID}/channels/{channelID}/messages?subtopic=humidity&from=1701000000&to=1701500000&limit=3" \
-H "Authorization: Bearer <token>"
