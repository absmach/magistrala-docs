...
<Callout type="info">
  Still current at a high level — `timescale-reader` and `postgres-reader` are real, separately
  deployed services (`cmd/timescale-reader`, `cmd/postgres-reader`, and both appear in
  `docker-compose.yaml`), matching [Architecture § Application & Data
  Services](/dev-guide/architecture#application--data-services)'s "Timescale Reader." The
  specifics below (query parameters, response shape) were not re-verified against current
  handlers in this pass — spot-check before publishing exact request/response examples.
</Callout>
...
cfg := timescale.Config{
    ...
    User: "magistrala", Pass: "magistrala",
