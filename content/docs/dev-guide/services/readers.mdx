...
<Callout type="info">
  `timescale-reader` and `postgres-reader` are real, separately deployed services — see
  [Architecture § Application & Data Services](/dev-guide/architecture#application--data-services)'s
  "Timescale Reader."
</Callout>
...
```bash
curl -X GET "http://localhost:8000/{workspaceID}/channels/{channelID}/messages?subtopic=temperature&from=1700000000&to=1700500000&limit=5" \
...
```bash
curl -X GET "http://localhost:8000/{workspaceID}/channels/{channelID}/messages?subtopic=humidity&from=1701000000&to=1701500000&limit=3" \
-H "Authorization: Bearer <token>"
