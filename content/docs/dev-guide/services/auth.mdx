...
<Callout type="warn" title="Correction: this page describes SpiceDB, which is gone; the Auth binary itself is not">
  Corrected after over-claiming in an earlier pass of this rewrite: `cmd/auth` still exists and
  still builds (it imports `pkg/atom` alongside its own Postgres/Redis/gRPC/tokenizer code), it's
  just absent from `docker-compose.yaml`'s default service list and from the Makefile's
  `SERVICES` — see [Getting Started § Build Individual
  ...
  **SpiceDB**: no SpiceDB container exists in `docker-compose.yaml`, and platform-wide identity/
  ...
  Whether/how the still-buildable `cmd/auth` binary is used today (standalone PAT issuance? legacy
  compat?) was not verified in this pass — the SpiceDB-specific content below is confirmed stale;
  the rest needs a closer look before being trusted.
</Callout>
