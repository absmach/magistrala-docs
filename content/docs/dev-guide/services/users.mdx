...
<Callout type="warn" title="This page describes a service that no longer exists standalone">
  Confirmed via `docker-compose.yaml` and `cmd/`: there is no separate Users service (or Auth
  service) in the current deployment. User accounts are now Atom entities of kind `human`, and
  authentication goes through Atom directly (`magistrala-cli login`) rather than a per-domain
  Users API. See [Architecture § Core Concepts](/dev-guide/architecture#core-concepts) for the
  current model. The content below documents the pre-Atom architecture and is kept for historical
  reference only — it does not describe how to manage users today.
</Callout>
