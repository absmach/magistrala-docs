...
<Callout type="warn" title="This page describes a service that no longer exists standalone">
  There is no separate Users service (or Auth service) in the current deployment. User accounts
  are now Atom entities of kind `human`, and authentication goes through Atom directly
  (`magistrala-cli login`) — see [Architecture § Core Concepts](/dev-guide/architecture#core-concepts)
  for the current model. The content below documents the pre-Atom architecture and is kept for
