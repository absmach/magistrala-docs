...
<Callout type="warn" title="Atom-backed service">
  Users are currently backed by Atom entities. The current Magistrala Docker configuration maps
  `MG_USERS_URL` to Atom's `/entities` endpoint, and there is no standalone `cmd/users` entry
  point in the Magistrala repository. The historical standalone build commands below are kept
  only for older deployments.
</Callout>
