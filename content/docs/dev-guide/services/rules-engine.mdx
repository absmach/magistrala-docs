...
4. **Secure, scoped execution** - Using workspace-level isolation and bearer token authorization. See the [Roles Schema](/dev-guide/roles-schema) for details on available roles and permissions.
...
> **Note:** Access to each operation is governed by workspace-level roles. Refer to the [Roles Schema](/dev-guide/roles-schema#rules) for details on which permissions are required.
