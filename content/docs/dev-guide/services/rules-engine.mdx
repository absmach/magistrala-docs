...
4. **Secure, scoped execution** - Using workspace-level isolation and bearer token authorization. See [Authorization](/dev-guide/authorization) for details on the roles and permission model.
...
> **Note:** Access to each operation is governed by workspace-level roles. Refer to [Authorization](/dev-guide/authorization) for details on which permissions are required.

### Broker Acknowledgment and Delivery

Rules Engine acknowledgment timing is configurable independently of rule execution success. This lets operators favor asynchronous consumption or wait for rule completion without necessarily retrying poison messages.

| Variable                       | Values            | Default  | Description                                                                                                                   |
| ------------------------------ | ----------------- | -------- | ----------------------------------------------------------------------------------------------------------------------------- |
| `MG_RE_ACK_MODE`               | `before`, `after` | `before` | Selects whether the broker can acknowledge before execution or must wait for every matched rule.                              |
| `MG_RE_ACK_ON_EXECUTION_ERROR` | `true`, `false`   | `true`   | In `after` mode, acknowledges execution errors when `true`; NACKs them for redelivery when `false`. Ignored in `before` mode. |

With `MG_RE_ACK_MODE=before`, the service validates the message, selects matching enabled rules, dispatches their executions concurrently, and returns to the broker without waiting for scripts or outputs. Script and output failures are logged but cannot trigger redelivery. This is the default and keeps slow rules or external outputs off the consumer callback's critical path.

With `MG_RE_ACK_MODE=after`, matching rules still execute concurrently, but the service waits for all of them before returning a broker disposition:

- `MG_RE_ACK_ON_EXECUTION_ERROR=true` reports script or output failures and acknowledges the message, preventing a failing rule from clogging the stream.
- `MG_RE_ACK_ON_EXECUTION_ERROR=false` NACKs the message when any matched rule fails and requests redelivery.

Redelivery reruns every matching rule, including rules that succeeded on the previous attempt. Configure rule outputs to be idempotent before enabling NACK-on-execution-error behavior. Repository and rule-selection failures occur before execution and remain retryable in either acknowledgment mode. Any `MG_RE_ACK_MODE` value other than `before` or `after` causes Rules Engine startup to fail.
...
| `workspace`     | Workspace ID associated with the rule.            | ✅              |
...
```go
type Alarm struct {
  Identity          // embeds ID; "type" on the wire comes from a custom MarshalJSON, not a literal field
...
```bash
m/<workspace_id>/c/<channel_id>/building1/floor2/temp
```
...
Endpoint: `POST /{workspaceID}/rules`

**Example command:**

```bash
curl --location 'http://localhost:9008/<your_workspace_id>/rules' \
...
```json title="Response"
{
  ...
  "name": "High Temperature Alert",
  "workspace": "3bf0d3e6-5383-4838-96a7-52c18da0aca0",
...
Enpoint: `Get /{workspaceID}/rules/{rule_id}`

**Example command:**

```bash
curl --location 'http://localhost:9008/<your_workspace_id>/rules/<your_rule_id>' \
--header 'Authorization: Bearer <your_access_token>'
```
...
Endpoint: `GET /{workspaceID}/rules`
...
```bash
curl --location 'http://localhost:9008/<your_workspace_id>/rules?input_channel=<your_channel_id>&status=enabled' \
--header 'Authorization: Bearer <your_access_token>'
```
...
Endpoint: `PATCH /{workspaceID}/rules/{rule_id}`

**Example command:**

```bash
curl --location --request PATCH 'http://localhost:9008/<your_workspace_id>/rules/<your_rule_id>' \
--header 'Content-Type: application/json' \
--header 'Authorization: Bearer <your_access_token>' \
--data '{
...
Endpoint: `PATCH /{workspaceID}/rules/{rule_id}/tags`

**Example command:**

```bash
curl --location --request PATCH 'http://localhost:9008/<your_workspace_id>/rules/<your_rule_id>/tags' \
--header 'Content-Type: application/json' \
--header 'Authorization: Bearer <your_access_token>' \
--data '{
  "tags": ["tag1","tag2"]
...
Endpoint: `PATCH /{workspaceID}/rules/{rule_id}/schedule`

**Example command:**

```bash
curl --location --request PATCH 'http://localhost:9008/<your_workspace_id>/rules/<your_rule_id>/schedule' \
--header 'Content-Type: application/json' \
--header 'Authorization: Bearer <your_access_token>' \
--data '{
    "schedule": {
...
Endpoint: `POST /{workspaceID}/rules/{rule_id}/disable`

**Example Command:**

```bash
curl --location --request POST 'http://localhost:9008/<your_workspace_id>/rules/<your_rule_id>/disable' \
--header 'Authorization: Bearer <your_access_token>' \
```
...
Endpoint: `POST /{workspaceID}/rules/{rule_id}/enable`

The API endpoint follows the format: `http://localhost:9008/{workspaceID}/rules/{ruleID}/enable`

```bash
curl --location --request POST 'http://localhost:9008/<your_workspace_id>/rules/<your_rule_id>/enable' \
--header 'Authorization: Bearer <your_access_token>' \
```
...
Endpoint: `DELETE /{workspaceID}/rules/{rule_id}`
**Example Command:**

```bash
curl --location --request DELETE 'http://localhost:9008/<your_workspace_id>/rules/<your_rule_id>' \
--header 'Authorization: Bearer <your_access_token>' \
```
