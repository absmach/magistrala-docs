...
```mermaid
flowchart LR
  Input[Channel input] --> Executor[Rule executor]
  Schedule[Schedule] --> Executor
  Executor --> Logic[Lua or Go logic]
  Logic --> Outputs{Outputs}
  Outputs --> Channel[Channel publisher]
  Outputs --> InternalDB[Internal DB]
  Outputs --> Postgres[External PostgreSQL]
  Outputs --> Alarm[Alarm]
  Outputs --> Email[Email]
  Outputs --> Slack[Slack]
```
