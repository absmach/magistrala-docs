...
<Callout type="info" title="Confirmed: Writers are real, Notifiers are orphaned code">
  There is no standalone `cmd/consumers` binary — `consumers/` is a library package, confirmed
  imported by `cmd/timescale-writer/main.go` (`consumers`, `consumers/tracing`,
  `consumers/writers/api`, `consumers/writers/timescale`) and, by the same pattern,
  `postgres-writer`. The **Writers** section below is accurate. The **Notifiers** section
  (SMTP/SMPP) is not: `consumers/notifiers/*` still exists as source but is only imported by its
  own subpackages, tests, and mocks — no `cmd/` binary or other service wires it up. It is dead
  code today, not a running feature. The actual current mechanism for the one notification
  Magistrala still sends (workspace-invitation emails) is Atom, not this package — see
  [Notifications](/dev-guide/services/notifications).
</Callout>
...
<Callout type="warn" title="Not currently wired up">
  Nothing in the current codebase imports this package outside of itself — it is not part of any
  running service. Kept below for reference only; do not treat it as a supported feature.
</Callout>
...
```go
import "github.com/absmach/magistrala/consumers/notifiers/smtp"
...
```go
import "github.com/absmach/magistrala/consumers/notifiers/smpp"
...
<Callout type="warn" title="Not currently wired up">
  Part of the same unused `consumers/notifiers` package — `consumers/notifiers/api/transport.go`
  exists but isn't mounted by any running service, so these endpoints are not actually reachable
  today.
</Callout>
