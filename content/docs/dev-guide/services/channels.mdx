...
<Callout type="warn" title="This page describes a service that no longer exists standalone">
  There is no separate Channels service. Channels are Atom resources of kind `channel`, managed
  through Atom's GraphQL API — see [Architecture § Core Concepts](/dev-guide/architecture#core-concepts)
  and the CLI's [Channels page](/dev-guide/cli/channels-cli). The content below documents the
  pre-Atom architecture and is kept for historical reference only.
</Callout>
