...
<Callout type="warn" title="Atom-backed service">
  Channels are currently backed by Atom resources, with Magistrala compatibility code in
  `pkg/channels` and `pkg/atom`. The current Docker configuration maps `MG_CHANNELS_URL` to Atom's
  `/resources` endpoint, and there is no standalone `cmd/channels` entry point in the Magistrala
  repository. The historical standalone build commands below are kept only for older deployments.
</Callout>
