...
```mermaid
flowchart TD
  UI[MG UI or API client] --> Config[Report configuration]
  Config --> Metrics[Metrics and filters]
  Config --> Schedule[Schedule]
  Metrics --> Reader[Message reader]
  Reader --> Store[(TimescaleDB / PostgreSQL)]
  Schedule --> Generator[Report generator]
  Store --> Generator
  Generator --> PDF[PDF export]
  Generator --> CSV[CSV export]
  Generator --> Email[Email delivery]
  Generator --> Download[Direct download]
```
