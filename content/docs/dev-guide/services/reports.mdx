...
> **Note:** All API operations require appropriate role-based permissions. Refer to [Authorization](/dev-guide/authorization) for details on the roles and permission model.
...
| WorkspaceID   | string        | Workspace context identifier             | ✅       |
...
| Metrics       | []ReqMetric   | List of metrics to include               | ✅       |
...
| DeviceID    | string  | Specific device/sensor ID                | Optional |
...
`Metric` is the shape a report configuration is *returned* as. When *creating or updating* one, the `metrics` array in the request body takes a different shape, `ReqMetric`, whose one difference is a `device_ids` array instead of a single `device_id`:

```go
type ReqMetric struct {
    ChannelID string    // Mandatory field
    DeviceIDs []string  // Optional field
    Name      string    // Mandatory field
    Subtopic  string    // Optional field
    Protocol  string    // Optional field
    Format    string    // Optional field
}
```
...
type MetricConfig struct {
    From        string      // Mandatory field. Relative start time (e.g., "now()-24h")
    To          string      // Mandatory field. Relative end time (e.g., "now")
...
|----------|-------------|---------------|
| `{{.Metric.DeviceID}}` | Device/client identifier | When filtering by specific devices |
...
type Metric struct {
    ...
    DeviceID  string // Device/sensor ID
...
```bash
curl --location --request PUT 'http://localhost:9017/{workspaceID}/reports/configs/{reportID}/template' \
...
```bash
curl --location 'http://localhost:9017/{workspaceID}/reports/configs/{reportID}/template' \
--header 'Authorization: Bearer $ACCESSTOKEN'
```
...
```bash
curl --location --request DELETE 'http://localhost:9017/{workspaceID}/reports/configs/{reportID}/template' \
--header 'Authorization: Bearer $ACCESSTOKEN'
```
...
```bash
curl --location 'http://localhost:9017/{workspaceID}/reports/configs' \
--header 'Content-Type: application/json' \
--header 'Authorization: Bearer $ACCESSTOKEN' \
--data '{
...
```bash
curl --location 'http://localhost:9017/{workspaceID}/reports?action=download' \
--header 'Content-Type: application/json' \
--header 'Authorization: Bearer $ACCESSTOKEN' \
--data '{
    "name": "Temp Report",
            ...
            <div class="metrics-section">
                <div class="metrics-info">
                    ...
                    <div class="metric-row">
                        ...
                        <div class="metric-value">{{.Metric.DeviceID}}</div>
                    </div>
                    {{end}}
                    <div class="metric-row">
...
- `{{.Metric.DeviceID}}` - Device ID (with conditional display).
...
{{range $index, $report := .Reports}}    <!-- Loop through reports -->
  ...
  {{if .Metric.DeviceID}}               <!-- Conditional display -->
    <div>{{.Metric.DeviceID}}</div>
  {{end}}
...
Base URL: `http://localhost:9017/{workspaceID}/reports`
...
```bash
{
    ...
    "workspace_id": "88c2bc9a-ce3b-4dfc-804d-219177cb9a75",
    "schedule": {
    ...
    "metrics": [
        {
            ...
            "device_id": "69a9b488-523a-4805-aae9-123febaf83f5",
            "name": "current",
            "format": ""
        }
    ],
...
```bash
curl -X POST http://localhost:9017/{workspaceID}/reports \
--header 'Content-Type: application/json' \
--header 'Authorization: Bearer $ACCESSTOKEN' \
--data '{
    "name": "lab 1 report",
    ...
    "reports": [
        {
            "metric": {
                ...
                "device_id": "e2ab8673-8599-435f-8d42-62903d279492",
                "name": "current",
                "format": ""
            },
            "messages": [
...
```bash
curl --location 'http://localhost:9017/bd1bb2c5-ce78-4456-8725-bd1beab80250/reports?action=download' \
--header 'Content-Type: application/json' \
--header 'Authorization: Bearer $ACCESSTOKEN' \
--data '{
    "name": "lab 1 report",
...
```bash
curl --location 'http://localhost:9017/bd1bb2c5-ce78-4456-8725-bd1beab80250/reports?action=email' \
--header 'Content-Type: application/json' \
--header 'Authorization: Bearer $ACCESSTOKEN' \
--data-raw '{
    "name": "lab 1 report",
...
```bash
curl "http://localhost:9017/{workspaceID}/reports/configs?status=enabled&limit=10" \
--header 'Authorization: Bearer $ACCESSTOKEN'
```
...
```bash
curl --location 'http://localhost:9017/{workspaceID}/reports/configs/{reportID}' \
...
Endpoint:
`POST /{workspaceID}/reports/configs/{reportID}/enable`

```bash
curl --location http://localhost:9017/{workspaceID}/reports/configs/{reportID}/enable \
--header 'Authorization: Bearer $ACCESSTOKEN'
```
...
Endpoint:
`POST /{workspaceID}/reports/configs/{reportID}/disable`

```bash
curl --location http://localhost:9017/{workspaceID}/reports/configs/{reportID}/disable \
--header 'Authorization: Bearer $ACCESSTOKEN'
```
...
Endpoint:
`PATCH /{workspaceID}/reports/configs/{reportID}`

```bash
curl --location --request PATCH 'http://localhost:9017/{workspaceID}/reports/configs/{reportID}' \
--header 'Content-Type: application/json' \
--header 'Authorization: Bearer $ACCESSTOKEN' \
--data '{
...
Endpoint:
`PUT /{workspaceID}/reports/configs/{reportID}/template`
...
Endpoint:
`GET /{workspaceID}/reports/configs/{reportID}/template`
...
Endpoint:
`DELETE /{workspaceID}/reports/configs/{reportID}/template`
