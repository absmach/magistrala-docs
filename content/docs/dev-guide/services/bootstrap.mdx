...
<Callout type="warn" title="Merged into atom-bootstrap">
  There is no standalone Bootstrap service anymore — it now runs merged with Atom as the
  `atom-bootstrap` deployment. The concept itself is still current (devices still bootstrap); the
  standalone-service specifics below (external ID/key encryption details, whitelist API) describe
  the pre-Atom service and are kept for historical reference only.
</Callout>
...
curl -s -S -i -X POST \
  ...
  http://localhost:9013/<workspaceID>/devices/configs \
  -d '{
...
The response body is the created config. The `id` is server-generated. `status` is always `enabled` on creation regardless of what is sent. `name` must be unique within a workspace — attempting to create a second config with the same name in the same workspace returns `409 Conflict`.
...
| --- | --- | --- |
| `POST` | `/{workspaceID}/devices/configs` | Create a config |
| `GET` | `/{workspaceID}/devices/configs` | List configs |
| `GET` | `/{workspaceID}/devices/configs/{config_id}` | View a config |
| `PATCH` | `/{workspaceID}/devices/configs/{config_id}` | Update editable config fields |
| `DELETE` | `/{workspaceID}/devices/configs/{config_id}` | Remove a config |
| `PATCH` | `/{workspaceID}/devices/configs/certs/{config_id}` | Update legacy certificate fields |
| `POST` | `/{workspaceID}/devices/configs/{config_id}/enable` | Enable device bootstrap for a config |
| `POST` | `/{workspaceID}/devices/configs/{config_id}/disable` | Disable device bootstrap for a config |
...
```http
/{workspaceID}/devices/bootstrap/profiles
```
...
curl -s -S -i -X POST \
  -H "Authorization: Bearer <user_token>" \
  -H "Content-Type: application/json" \
  http://localhost:9013/<workspaceID>/devices/bootstrap/profiles \
  -d '{
...
curl -s -S -i -X POST \
  ...
  http://localhost:9013/<workspaceID>/devices/bootstrap/profiles/upload \
...
curl -s -S \
  -H "Authorization: Bearer <user_token>" \
  http://localhost:9013/<workspaceID>/devices/bootstrap/profiles/<profile_id>/slots
```
...
curl -s -S -X POST \
  -H "Authorization: Bearer <user_token>" \
  -H "Content-Type: application/json" \
  http://localhost:9013/<workspaceID>/devices/bootstrap/profiles/<profile_id>/render-preview \
  -d '{
    "config": {
      "id": "preview-config",
      "external_id": "01:6:0:sb:sa",
...
| Method | Path | Description |
| --- | --- | --- |
| `POST` | `/{workspaceID}/devices/bootstrap/profiles` | Create a profile |
| `POST` | `/{workspaceID}/devices/bootstrap/profiles/upload` | Upload a JSON, YAML, or TOML profile |
| `GET` | `/{workspaceID}/devices/bootstrap/profiles` | List profiles |
| `GET` | `/{workspaceID}/devices/bootstrap/profiles/{profile_id}` | View a profile |
| `GET` | `/{workspaceID}/devices/bootstrap/profiles/{profile_id}/slots` | List profile binding slots |
| `POST` | `/{workspaceID}/devices/bootstrap/profiles/{profile_id}/render-preview` | Preview rendered profile output |
| `PATCH` | `/{workspaceID}/devices/bootstrap/profiles/{profile_id}` | Update a profile |
| `DELETE` | `/{workspaceID}/devices/bootstrap/profiles/{profile_id}` | Remove a profile |
...
```http
/{workspaceID}/devices/bootstrap/enrollments
```

In this path, `enrollment` means the Bootstrap config. There is no separate create-enrollment endpoint; create configs with `POST /{workspaceID}/devices/configs`.
...
curl -s -S -i -X PATCH \
  -H "Authorization: Bearer <user_token>" \
  -H "Content-Type: application/json" \
  http://localhost:9013/<workspaceID>/devices/bootstrap/enrollments/<config_id>/profile \
  -d '{
    "profile_id": "77708c94-3ff5-4068-ab5b-72d1318a9f1c"
  }'
```
...
curl -s -S -i -X PUT \
  -H "Authorization: Bearer <user_token>" \
  -H "Content-Type: application/json" \
  http://localhost:9013/<workspaceID>/devices/bootstrap/enrollments/<config_id>/bindings \
  -d '{
    "bindings": [
      {
        "slot": "mqtt_client",
...
| Method | Path | Description |
| --- | --- | --- |
| `PATCH` | `/{workspaceID}/devices/bootstrap/enrollments/{config_id}/profile` | Assign a profile to a config |
| `PUT` | `/{workspaceID}/devices/bootstrap/enrollments/{config_id}/bindings` | Bind resources and store snapshots |
| `GET` | `/{workspaceID}/devices/bootstrap/enrollments/{config_id}/bindings` | List binding snapshots |
| `POST` | `/{workspaceID}/devices/bootstrap/enrollments/{config_id}/bindings/refresh` | Refresh stored binding snapshots |
...
<Callout type="warn" title="This section describes an outdated protocol">
  The device-facing endpoints below (`GET /clients/bootstrap/...`, static AES-key "Secure
  Bootstrap") no longer match the registered routes. The current code exposes a two-step
  challenge/response flow instead: `POST /devices/bootstrap/challenges/{externalID}` followed by
  `POST /devices/bootstrap/configurations/{externalID}` (see `bootstrap/api/transport.go`,
  `bootstrap/device_bootstrap.go`, and `bootstrap/device_crypto.go`), using
  `BootstrapChallengeID`/`BootstrapServerNonce`/`BootstrapDeviceNonce`/`BootstrapRootKey` rather
  than a single `MG_BOOTSTRAP_ENCRYPT_KEY`. This section needs a rewrite from someone familiar
  with the new nonce-based handshake — left as-is for now rather than guessing at the wire format.
</Callout>
...
Use these examples as quick smoke tests for every Bootstrap endpoint. Replace `<workspace_id>`, `<config_id>`, `<profile_id>`, `<external_id>`, `<external_key>`, `<encrypted_external_key>`, and `<user_token>` with real values.
...
curl -s -S \
  -H "Authorization: Bearer <user_token>" \
  'http://localhost:9013/<workspaceID>/devices/configs?offset=0&limit=10'
```
...
curl -s -S \
  -H "Authorization: Bearer <user_token>" \
  'http://localhost:9013/<workspaceID>/devices/configs?offset=0&limit=10&status=all'
```
...
curl -s -S \
  -H "Authorization: Bearer <user_token>" \
  http://localhost:9013/<workspaceID>/devices/configs/<config_id>
```
...
curl -s -S -i -X PATCH \
  -H "Authorization: Bearer <user_token>" \
  -H "Content-Type: application/json" \
  http://localhost:9013/<workspaceID>/devices/configs/certs/<config_id> \
  -d '{
...
curl -s -S -i -X POST \
  -H "Authorization: Bearer <user_token>" \
  http://localhost:9013/<workspaceID>/devices/configs/<config_id>/enable
```
...
curl -s -S -i -X POST \
  -H "Authorization: Bearer <user_token>" \
  http://localhost:9013/<workspaceID>/devices/configs/<config_id>/disable
```
...
curl -s -S \
  -H "Authorization: Bearer <user_token>" \
  'http://localhost:9013/<workspaceID>/devices/bootstrap/profiles?offset=0&limit=10'
```
...
curl -s -S \
  -H "Authorization: Bearer <user_token>" \
  http://localhost:9013/<workspaceID>/devices/bootstrap/profiles/<profile_id>
```
...
curl -s -S -i -X POST \
  -H "Authorization: Bearer <user_token>" \
  http://localhost:9013/<workspaceID>/devices/bootstrap/enrollments/<config_id>/bindings/refresh
```
...
curl -s -S -i -X PUT \
  -H "Authorization: Bearer <user_token>" \
  -H "Content-Type: application/json" \
  http://localhost:9013/<workspaceID>/devices/bootstrap/enrollments/3094cf29-e73f-4be1-a4d2-b5c5d7a827ec/bindings \
  -d '{
    "bindings": [
      {
        "slot": "mqtt_client",
...
curl -s -S -X POST \
  -H "Authorization: Bearer <user_token>" \
  -H "Content-Type: application/json" \
  http://localhost:9013/<workspaceID>/devices/bootstrap/profiles/77708c94-3ff5-4068-ab5b-72d1318a9f1c/render-preview \
  -d '{
    "config": {
      "id": "3094cf29-e73f-4be1-a4d2-b5c5d7a827ec",
      "external_id": "rpi-warehouse-1-temp-001",
...
curl -s -S -i -X POST \
  -H "Authorization: Bearer <user_token>" \
  http://localhost:9013/<workspaceID>/devices/configs/3094cf29-e73f-4be1-a4d2-b5c5d7a827ec/enable
```
...
- Configs are workspace-owned records, reached under `/{workspaceID}/devices/configs`. There is no `/clients/configs` compatibility alias — that path was renamed, not kept.
...
- Binding a `channel` resource automatically connects the enrolled device to that channel in Magistrala. No separate connection step is needed.
