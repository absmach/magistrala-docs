...
<Callout type="warn">
The OPC-UA adapter is a `magistrala-contrib` service and has been obsolete for a while (see the [magistrala-contrib overview](/dev-guide/extensions/magistrala-contrib)). Also uses pre-rename `client`/`Client` terminology throughout.
</Callout>
