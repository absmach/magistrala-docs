...
The API/CLI for interacting with Devices is in the [Devices CLI](/dev-guide/cli/devices-cli); Channels in the [Channels CLI](/dev-guide/cli/channels-cli); Gateways in the [Gateways CLI](/dev-guide/cli/gateways-cli); Device Types in the [Device Types CLI](/dev-guide/cli/devicetypes-cli).
...
Entity/channel/group activity (creation, updates, disabling, connectivity, role changes) is tracked by **Atom's own audit log**, exposed via a GraphQL query (`magistrala-ui`'s `packages/ui/src/lib/journal.ts`, which the UI's per-entity **Journals** tab calls):
...
<Callout type="warn" title="Atom-backed deployments only">
  This query is only available when the deployment targets Atom (`MG_ATOM_URL` set). Deployments
  without Atom configured do not have this audit log available.
</Callout>
