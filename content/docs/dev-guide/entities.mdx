...
There is no dedicated `users` CLI command in the current CLI (verified against `cli/root.go`'s `NewRootCmd()`) — user accounts are managed through Atom; see the [CLI Introduction](/dev-guide/cli/introduction-to-cli) for what's actually available.
...
[clients-cli]: /dev-guide/cli/devices-cli
[channels-cli]: /dev-guide/cli/channels-cli
[domains-cli]: /dev-guide/cli/workspaces-cli
