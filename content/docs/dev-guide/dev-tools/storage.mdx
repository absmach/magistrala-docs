...
<Callout type="info">
PostgreSQL and Timescale (`postgres-writer`/`postgres-reader`, `timescale-writer`/`timescale-reader`)
are the storage backends shipped in `docker/addons/` today. The Cassandra, MongoDB, and InfluxDB
sections below describe add-ons from an earlier release and aren't part of the current `docker/addons/`.
</Callout>
