...
<Callout type="warn">
The sections below (MQTT Broker, Message Broker, and their NATS/RabbitMQ/Kafka subsections)
describe the previous pluggable-broker model. **FluxMQ** is now Magistrala's default message
broker for both MQTT and general message exchange — NATS remains available only as a build-time
alternative. See [Architecture](/dev-guide/architecture) and
[Storage Architecture](/dev-guide/storage-architecture) for the current model. The content below
