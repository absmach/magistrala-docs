...
<Callout type="info">
**FluxMQ** is Magistrala's default message broker for both MQTT and general message exchange — see
[Architecture](/dev-guide/architecture) and [Storage Architecture](/dev-guide/storage-architecture)
for the current model. The sections below (MQTT Broker, Message Broker, and their NATS/RabbitMQ/Kafka
subsections) describe the pluggable-broker configuration for build-time alternatives to FluxMQ.
</Callout>
