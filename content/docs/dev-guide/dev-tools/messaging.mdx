...
for the current model. There is no pluggable-broker configuration: `MG_BROKER_TYPE`,
`MG_MQTT_BROKER_TYPE`, and the NATS/RabbitMQ/Kafka environment variables this section used to
describe don't exist anywhere in the current codebase — `pkg/messaging/brokers/` contains only the
FluxMQ implementation. Magistrala does depend on `rabbitmq/amqp091-go`, but as an AMQP 0.9.1 client
for talking to FluxMQ itself, not as support for an alternative RabbitMQ broker.
