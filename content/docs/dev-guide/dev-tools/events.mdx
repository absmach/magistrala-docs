...
[Architecture](/dev-guide/architecture) — while the Redis Streams mechanism below covers the
`mqtt` adapter's own event log.
</Callout>

In order to be easily integrable system, Magistrala uses [Redis Streams][redis-streams] as an event log for event sourcing, currently used by the `mqtt` adapter.
