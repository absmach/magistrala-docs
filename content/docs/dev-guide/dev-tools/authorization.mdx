...
<Callout type="warn">
This page describes the previous SpiceDB-backed authorization model (`domains`/`clients` terminology, SpiceDB schema language). SpiceDB has been replaced — authorization now goes through **Atom**, and the entities are **Workspaces**/**Devices** (see `docs/latest-dev-guide`'s `authorization.mdx` for the current model). This page needs a full rewrite against Atom's actual authorization implementation, not a terminology find-and-replace.
</Callout>
