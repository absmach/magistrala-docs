...
<Callout type="warn">
Everything below this point describes a pre-rename architecture (`magistrala-cli` with `users create`/`clients` subcommands, `Authorization: Client` headers against `/clients/bootstrap/`, standalone `github.com/absmach/agent` and `github.com/absmach/export` repos, `gnatsd`) that does not match the current CLI (`health, login, workspaces, channels, groups, authz, devices, gateways, devicetypes` — no `provision`/`users create`/`clients` commands exist) or the current edge architecture. Neither an `agent` nor `export` repo exists locally to verify the current design against, and `edge/architecture.md` treats "the agent" as an external black-box component rather than something this repo implements — so this page's specifics need review by someone with access to the current `absmach/agent`/`absmach/export` repos before Latest ships it. The general concept (a gateway running local services that bridge to Magistrala) is likely still directionally correct; the commands, env vars, and API paths below are not.
</Callout>
