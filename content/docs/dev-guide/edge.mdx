...
<Callout type="warn">
Everything below this point describes a pre-rename architecture (`magistrala-cli` with `users create`/`clients` subcommands, `Authorization: Client` headers against `/clients/bootstrap/`, `gnatsd`) that does not match the current CLI (`health, login, workspaces, channels, groups, authz, devices, gateways, devicetypes` — no `provision`/`users create`/`clients` commands exist) or the current edge architecture. `github.com/absmach/agent` and `github.com/absmach/export` are confirmed to be real, separate repos (not available locally in this pass) — `edge/architecture.md` treats "the agent" as an external black-box component rather than something this repo implements, which is consistent with that. This page's specifics still need review by someone with access to those repos' current state before Latest ships it. The general concept (a gateway running local services that bridge to Magistrala) is likely still directionally correct; the commands, env vars, and API paths below predate the Workspaces/Devices rename and are not.
</Callout>
...
- Heartbeat - listening to Message Broker topic `heartbeat.>` it can remotely provide info on running services, if services are publishing heartbeat ( like [Export](/dev-guide/edge#export))
...
[mutual-tls]: /dev-guide/dev-tools/authentication#mutual-tls-authentication-with-x509-certificates
[certs-service]: /dev-guide/certs#certs-service
[protomsg]: https://github.com/absmach/magistrala/blob/main/pkg/messaging/message.proto
[back-to-edge]: /dev-guide/edge
[nats]: https://nats.io/
