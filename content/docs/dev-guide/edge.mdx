...
<Callout type="warn">
This page describes the pre-rename architecture (`magistrala-cli` with `users create`/`clients`
subcommands, `Authorization: Client` headers, `gnatsd`) and predates the current CLI (`health,
login, workspaces, channels, groups, authz, devices, gateways, devicetypes`). `Agent` and `Export`
are maintained in their own separate repositories, [`github.com/absmach/agent`](https://github.com/absmach/agent)
and [`github.com/absmach/export`](https://github.com/absmach/export) — the general concept
(a gateway running local services that bridge to Magistrala) still applies; the commands, env
vars, and API paths below are kept for historical reference only.
</Callout>
