...
<Callout type="info">
`Agent` and `Export` are maintained in their own separate repositories,
[`github.com/absmach/agent`](https://github.com/absmach/agent) and
[`github.com/absmach/export`](https://github.com/absmach/export). The general concept described
here — a gateway running local services that bridge to Magistrala — still applies; check those
repositories for the current CLI commands, env vars, and API paths, since the examples below use
an older client model.
</Callout>
