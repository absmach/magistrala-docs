...
Magistrala is built on top of [FluxMQ](https://github.com/absmach/fluxmq), a message broker designed for both messaging and durable event streams, and on [Atom](https://github.com/absmach/atom), a lightweight identity-and-authorization service that is the system of record for workspaces, users, devices, channels, groups, roles, and policies. Magistrala itself provides everything around them: protocol adapters, device provisioning, data processing, and observability.
...
**Atom** is Magistrala's identity, authorization, and catalog service. It runs as its own deployment with its own PostgreSQL database, and is the system of record for workspaces, users, devices, channels, groups, roles, and policies. It issues the JWTs and JWKS that every other service validates on every request. See [Core Concepts](#core-concepts) below for how Magistrala's concepts map onto Atom's.
...
| Device              | Entity, kind `device`      | A device or application that sends/receives data                      |
| Channel             | Resource, kind `channel`   | A messaging/data path that devices can publish or subscribe to        |
...
FluxMQ is Magistrala's messaging and event-streaming backbone — there are no NATS references anywhere in the current codebase (`go.mod`, `Makefile`, `pkg/messaging/brokers/`), so it is not a build-time alternative today. There is no constraint on the content exchanged through a channel, but messages should be formatted using [SenML][senml] so they can be post-processed and normalized.
