...
FluxMQ is Magistrala's messaging and event-streaming backbone. There is no constraint on the content exchanged through a channel, but messages should be formatted using [SenML][senml] so they can be post-processed and normalized.
