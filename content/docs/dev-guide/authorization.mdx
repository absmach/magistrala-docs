...
`ScopeMode` is one of nine values: the whole platform; the tenant itself; all objects of a kind; all objects of a type; one exact object (by `ObjectKind`/`ObjectType`/`ObjectID`); direct objects in a group; objects in subgroups; direct child groups; or descendant groups.
