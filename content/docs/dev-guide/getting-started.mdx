...
Magistrala depends on several infrastructural services, notably [FluxMQ][fluxmq] for message transport and event streams, Atom with PostgreSQL for identity and authorization, and TimescaleDB/PostgreSQL for message storage. The Docker deployment also includes supporting infrastructure such as NATS where required by internal components.
...
Magistrala's service-level authorization schema is loaded from a single, directly edited [SpiceDB](https://authzed.com/spicedb) schema file:
...
[provisioning]: /dev-guide/cli/provision-cli
