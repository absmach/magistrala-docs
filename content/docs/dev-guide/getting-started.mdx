...
<Callout type="warn" title="Partially stale — verified fixes applied, some example output not yet re-captured">
  The install/build/run instructions on this page were spot-checked against the current
  `Makefile` and `cmd/` in this pass (see inline corrections below). The example command *output*
  further down — service log lines like `magistrala-users`, `magistrala-clients`,
  `magistrala-broker`, and the `SpiceDB Schema Management` section — predates the Atom migration
  and was not re-captured from a live run; treat those specific blocks as illustrative history,
  not current behavior, until someone re-runs them against today's stack.
</Callout>
...
<Callout type="warn" title="This command no longer exists">
  ...
  no `provision` command in the current CLI (verified against `cli/root.go`'s `NewRootCmd()` — see
  [CLI Introduction](/dev-guide/cli/introduction-to-cli)). To provision a workspace with devices
  and channels today, use [`workspaces create`](/dev-guide/cli/workspaces-cli),
...
<Callout type="warn" title="This command no longer exists">
  There is no `messages` command in the current CLI (verified against `cli/root.go`'s
  `NewRootCmd()`). Send messages over the platform's protocol adapters (HTTP, MQTT, CoAP,
  WebSocket) directly instead — see [Architecture § Who Connects](/dev-guide/architecture#who-connects).
  The example below is kept for historical reference only.
</Callout>
...
<Callout type="warn" title="This section describes a service that no longer exists">
  Confirmed via `docker-compose.yaml` and `cmd/`: there is no SpiceDB deployment. Authorization is
  now provided by Atom (see [Architecture § Identity & Access
  Control](/dev-guide/architecture#identity--access-control)) — `docker/permission.yaml` still
  exists in the repository and may be relevant to Atom's setup, but the SpiceDB-specific schema
  workflow below (`docker/spicedb/schema.zed`) was not re-verified in this pass and should be
  checked before use.
</Callout>
