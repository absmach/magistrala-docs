...
<Callout type="warn">
  The example command *output* further down this page — service log lines like
  `magistrala-users`, `magistrala-clients`, `magistrala-broker`, and the `SpiceDB Schema
  Management` section — predates the Atom migration. Treat those specific blocks as illustrative
  history, not current output.
</Callout>
...
<Callout type="warn" title="NATS is no longer the default broker">
  FluxMQ is now Magistrala's default message broker; NATS is only available as a build-time
  alternative. The override mechanism shown below still applies, but this specific example
  targets the legacy `nats` service and won't apply to a default FluxMQ setup.
</Callout>
...
<Callout type="warn" title="This command no longer exists">
  `magistrala-cli provision test` and the example output below are from the pre-Atom CLI. There is
  no `provision` command in the current CLI. To provision a workspace with devices and channels
  today, use [`workspaces create`](/dev-guide/cli/workspaces-cli),
  [`devices create`](/dev-guide/cli/devices-cli), and [`channels
  create`](/dev-guide/cli/channels-cli) directly.
</Callout>

Output of the old command followed this pattern (kept for historical reference only):
...
<Callout type="warn" title="This command no longer exists">
  There is no `messages` command in the current CLI. Send messages over the platform's protocol
  adapters (HTTP, MQTT, CoAP, WebSocket) directly instead — see [Architecture § Who
  Connects](/dev-guide/architecture#who-connects). The example below is kept for historical
  reference only.
</Callout>

Once system is provisioned, a device can start sending messages on a channel. The old command looked like:
...
<Callout type="warn" title="FluxMQ is now the default message broker">
  This section describes the previous NATS-based local dev setup. **FluxMQ** is now Magistrala's
  default message broker; NATS remains available only as a build-time alternative. See
  [Architecture](/dev-guide/architecture) for the current model. The content below is kept for
...
<Callout type="warn" title="This section describes a service that no longer exists">
  There is no SpiceDB deployment. Authorization is now provided by Atom — see
  [Architecture § Identity & Access Control](/dev-guide/architecture#identity--access-control).
  The SpiceDB schema workflow below is kept for historical reference only.
</Callout>
