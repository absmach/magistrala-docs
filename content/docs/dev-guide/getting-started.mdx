...
<Callout type="info">
  This example overrides the `nats` service, which applies only when Magistrala is built with NATS
  as the broker. FluxMQ is the default message broker — see [Message Broker](#message-broker) — so
  adapt the override to the `fluxmq` service for a default setup.
</Callout>
...
[FluxMQ](https://github.com/absmach/fluxmq) is Magistrala's default message broker and event
stream, handling MQTT, WebSocket, HTTP, CoAP, and AMQP protocol adapters — see
[Architecture](/dev-guide/architecture) for the full model. NATS remains available as a build-time
alternative for local development.

To run NATS instead (when not running via Docker), install and start it locally:

```bash
go install github.com/nats-io/nats-server/v2@latest
```

This will install the `nats-server` binary, which can be run directly:
...
To use an alternative broker (e.g., NATS, RabbitMQ, or VerneMQ) at build time, set the env
variable:
