...
<Callout type="warn" title="Standalone CLI build is not currently wired">
  ...
  binary. Use released CLI artifacts if they are available for your target release; otherwise
  treat the standalone CLI examples below as historical until a new build target is restored.
</Callout>
