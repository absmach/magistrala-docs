...
Available Commands:
  ...
  password     Manage the authenticated user's password
  users        Users management
  workspaces   Manage Magistrala workspaces through Atom tenants

Flags:
      --graphql-url string   Atom GraphQL endpoint
...
(Reconstructed from `cli/root.go`'s command registration and persistent flags, not captured from a running binary. `--graphql-url` and `--token` have no shorthand. Verify with your own `--help` run before relying on exact flag text.)
