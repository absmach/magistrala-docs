...
- **Members** – Shows the total number of users assigned to the domain, along with their enabled or disabled status.  
  The domain creator is automatically assigned the **tenant-admin** role and appears as the first active member.
...
The **Domain Homepage** serves as a central dashboard for domain-level monitoring, configuration, and resource management, providing both high-level insights and quick access to detailed sections.
...
By default, a **tenant-admin** role with complete control over the domain is always present and granted to the Domain creator. The Roles table lists each role's **Name**, **Created By**, **Created At**, **Updated By** and **Updated At**.
...
Once created, domain roles can be edited from their respective pages. A role's page shows its **Name** and **ID**, its **Permission scopes** (each scope card lists its actions and has buttons to **Add actions**, **Remove actions** or **Delete scope**, plus an **Add scope** button to add another permission block), and an **Assignments** table of the users currently holding the role, with **Add Members** and **Delete All Members** actions.
