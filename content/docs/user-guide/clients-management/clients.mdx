...
You can also create multiple clients at once by clicking on the `Upload` button and uploading a _.csv_ file with client **names** and any other fields you would like to add.
...
Clicking on the **Assign Member** button will open a dialog box that allows the user to select amongst the Workspace Members present as well as the Roles present in the Client.
