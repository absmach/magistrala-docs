...
For detailed instructions on each protocol, see the [Messaging Guide](/docs/dev-guide/dev-tools/messaging).

### Saving Messages

For a message to be saved on the Magistrala database, a **Rule** must be created in Rules Engine in relation to the specific channel and topic. This has been further discussed in the [**Rules Engine** documentation](/docs/user-guide/rules-engine).
