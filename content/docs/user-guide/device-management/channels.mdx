...
**Channels** act as message conduits. They are responsible for the messages exchange between devices and act as message topics that can be **published** or **subscribed** to by multiple Devices.

Devices are able to **publish** or **subscribe** to a channel depending on their connection.

## Create a Channel

To create a channel, click on the `Create` button. You can also create multiple channels by clicking on the `Upload` button and uploading a `.csv` file.

![Create channel](../../img/devices/channel-create-buttons.png)

### Channel Information

Add a channel **name** and optionally a **Route**, **tags** and **metadata**. Like a workspace's Route, the channel **Route** is a short, human-readable identifier for the channel.

![Channel information](../../img/devices/channel-create.png)
...
![Bulk create channels](../../img/devices/channels-bulk-create.png)
...
![View channel](../../img/devices/channel-view.png)
...
- **Route** – A user-friendly identifier for the channel, set at creation time
...
![Channel Settings](../../img/devices/channel-settings.png)
...
![Channel Metadata](../../img/devices/channel-metadata.png)

## Connect to a Device

Navigate to the **Connections** tab on the channel sidebar. This will lead you to the connections page where you can view the devices connected to a channel.

![Connections page](../../img/devices/channel-connections.png)

In this page you are able to add a connection by clicking on the `Connect` button on the top right corner.

A dialog will open where you can select devices and connection types. A device can have both connection types selected.  
...
![Connect device](../../img/devices/channel-connect-client.png)

To disconnect the device, click on the `Disconnect` button at the end of the channel row.

![Disconnect device button](../../img/devices/channel-disc-client.png)
...
![Disconnect device dialog](../../img/devices/channel-disc-client-dialog.png)

## Channel Roles

Roles define a set of actions that can be allocated to users.

Navigate to the **Roles** tab of the channel sidebar. This will lead you to a table of all the available roles in the channel, listing each role's **Name**, **Created By**, **Created At**, **Updated By** and **Updated At**.

To create a role, navigate to the roles section on the channel navbar. Click on the `Create` button and provide a role name. The actions and members are optional fields.

![Create channel role](../../img/devices/channel-create-role.png)

### Channel Role Information

![Channel role information](../../img/devices/channel-create-role-dialog.png)
...
Click a role in the **Roles table** to open its page. The page shows the role's **Permissions** (its selected actions) and an **Assignments** table of the members holding the role.
...
![Update role name](../../img/devices/channel-role-update.png)
...
![Update role actions](../../img/devices/channel-role-add-actions.png)

### Update Channel Role Members

Members are listed in a separate table below the **Role Actions** section.

To add new members to the channel under the current role click on `Add Members` button. The pop-up dialog will allow you to add any user that is a member of the workspace to the channel. The users will show up on a dropdown menu.

![Channel role add members](../../img/devices/channel-role-add-members.png)
...
![Delete role actions](../../img/devices/channel-role-delete-actions.png)
...
![Delete role members](../../img/devices/channel-role-delete-members.png)

To delete specific members from the Role Members Table, click on the **trash** icon.

![Delete role member](../../img/devices/channel-role-member-delete.png)
...
![View role members](../../img/devices/channel-members.png)

### Assign Channel Members

Members can be assigned to a channel by clicking on the `Assign Member` button.
This will open up a dialog that allows you to select from amongst the Workspace Members present from a dropdown list.
The roles presented will also be the very same Channel Roles from above.

![Assign role members](../../img/devices/channel-assign-member.png)
...
![Unassign role members](../../img/devices/channel-members-unassign.png)

## Messages

While on channels, a user can view, send and download messages associated to the channel.

![Messages Page](../../img/devices/empty-messages-page.png)

### Send Messages

The **Send Message** feature allows you to publish sensor or device data directly from the Magistrala UI.  
A connected device acts as the publisher, and the message can be sent over different supported protocols (**HTTP**, **MQTT**, **CoAP**, **WebSocket**).
...
4. **Publisher** - This is the **connected** Device which can be selected from an infinite select. From this device the backend will get the special _Device-Key_ which will be used to send the message.
...
![Send Message](../../img/devices/send-messages.png)
...
For detailed instructions on each protocol, see the [Messaging Guide](/dev-guide/dev-tools/messaging).
...
For a message to be saved on the Magistrala database, a **Rule** must be created in Rules Engine in relation to the specific channel and topic. This has been further discussed in the [**Rules Engine** documentation](/user-guide/rules-engine/overview).

The messages table will then update to include the message sent with the latest message appearing first.

![Messages Table](../../img/devices/messages-table.png)
...
The `Authorization` header keyword is `Client` regardless of the publishing device's type — this is a fixed wire-protocol prefix and isn't renamed alongside the Device entity.
...
![Filter Messages](../../img/devices/filter-panel.png)  

You can filter messages by:

- **Protocol** – Select the communication protocol used (e.g., MQTT, HTTP).
- **Publisher** – Choose the device that published the message.
...
![Download Messages Dialog](../../img/devices/download-messages.png)
...
- **Publisher** – Filter by device who published the messages.
...
The **Journals** tab (formerly named **Audit Logs**) tracks all **channel-related events**, including **creation**, **updates**, **disabling**, **connectivity** and **role changes**.
...
![Channel Audit Logs](../../img/devices/channel-logs.png)
...
![View Channel Audit Log Actions](../../img/devices/channel-audit-action-button.png)
...
![Delete Channel](../../img/devices/channel-delete.png)
