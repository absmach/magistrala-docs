...
| `device_id`       | ID of the Device associated with the alarm                | ✅ (added by system) |
...
| `severity`        | Severity level of the alarm, 0-100 (e.g., 10 = low, 90 = critical) | ✅                   |
...
Severity values range from 0 to 100, with 0 being the lowest and 100 the highest. The scale is otherwise user-defined — pick whatever thresholds within that range make sense for your rules.
...
2. Select from available filter options:
   ...
   - **Client**: Filter by the device or client that triggered the alarm
...
ID, Severity, Cause, Value, Threshold, Device ID, Assignee, Assigned At, Acknowledged At, Resolved At, Updated At, Rule, Status, Measurement, Unit, Channel ID, Subtopic, Assigned By, Acknowledged By, Resolved By, Updated By, Created At
