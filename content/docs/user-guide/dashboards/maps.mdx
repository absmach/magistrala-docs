...
1. **Entity Type**: Select the type of entity to visualize (**Device**, **Channel**, **Group**).
...
Route Maps visualize location data by retrieving it from messages sent through a channel. The process for sending messages is covered in [getting-started](/dev-guide/getting-started).
To use Route Maps, navigate to the messages tab of a channel that has connected devices. For Route Maps, it is crucial to use `location` as the value name, and the value type must be string. The message should contain the location data in the following format (as a string):
...
3. **Device**: Choose the connected device associated with the selected channel.
...
7. **Add Source**: You can add multiple data sources (devices or channels) by clicking **Add Source**.
...
1. **Add or Delete Data Sources**: Add additional channels or devices to the map, or remove existing ones by clicking `Add Source` or the `trash` icon, respectively.
...
- **View Details**: A link to the specific entity's details page.
...
2. **Entity Type**: Choose whether to visualize **Devices** or **Channels** within the group.  
3. **Entities**: Select specific **Group Devices** or **Group Channels** that contain valid location metadata.  
...
- **Entity Name**: Name of the device or channel with an identifying icon.  
