...
The **Switch Card** allows a user to send boolean values (**true** or **false**) to a connected device via a channel. This is useful for devices that support simple on/off behavior, such as relays, motors, or lights.
...
- **Device (*Required*)** - The target device receiving the command.
...
The default state is **OFF**. Toggle the control to **ON** to send a `true` command to the device.
...
- **Device** - Select a different connected device.
...
- **Device** (Required) – The target device receiving the value.
...
Drag the handle to select a value. The new value is sent immediately to the target device.
