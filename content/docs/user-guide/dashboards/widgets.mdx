...
In **Control widgets**, sliders require minimum and maximum values. The step value defines the increment or decrement for each movement — it has no default, so set one explicitly for the slider to move in meaningful increments.
