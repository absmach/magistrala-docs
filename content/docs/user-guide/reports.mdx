...
|Start Time | How far back the report data starts, relative to now (e.g. `1h`, `10h`, `1d`). The actual start time is calculated as `now() - Start Time`.| ✅ |
|End Time | How far back the report data ends, relative to now (e.g. `1h`, `10h`, `1d`). The actual end time is calculated as `now() - End Time`. Defaults to now if left empty.| Optional |
...
|Recurring Period | How many intervals to skip between executions (e.g., 1 = every interval, 2 = every second interval, etc.).| Required if Recurring Interval is not `None` |
