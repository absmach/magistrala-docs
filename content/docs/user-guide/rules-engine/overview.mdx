...
- **Logic Nodes**: Define conditions using a comparison block, or a Code Editor node that toggles between Lua and Go scripts.
- **Output Nodes**: Output processed data via Channel Publisher, Email, PostgreSQL, Internal DB, Alarm, or Slack.

### 3. Scripting and Templating

- **Code Editor**: A single logic node with a language selector — write Lua with a required `logicFunction()`, or switch to Go for more advanced use cases (requires Go syntax compliance).
...
To create a new rule, navigate to the **Rules** section and click the `Create Rule` button.
...
After setting the input, define your rule’s logic using one of two options:

1. Comparison Block
2. Code Editor (toggle between Lua and Go — see [Script Editors](#script-editors))
...
An alarm output will enable you to be able to generate alarms in the case where a threshold has been exceeded. More information about this node is provided in the [Alarms](/user-guide/alarms) section.
...
To create a role, navigate to the **Roles** tab on the rule sidebar. Click on the `Create` button and provide a role name. The actions and members are optional fields. The **Roles table** lists each role's **Name**, **Created By**, **Created At**, **Updated By** and **Updated At**.
...
> Unlike devices, channels and groups, rule roles use a larger, rule-specific action set — it includes
> `execute` plus separate actions for managing the alarms a rule's output can create, on top of the
> platform's generic vocabulary:
>
> - read
> - write
> - delete
...
To update the role's permissions, use the **Add actions** / **Remove actions** buttons. A dialog box will appear allowing you to select the actions you want to add.
