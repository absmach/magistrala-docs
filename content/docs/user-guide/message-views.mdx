...
Messages only appear on the Messages page if a [Rule](/user-guide/rules-engine) is set up to store them in Magistrala's internal database — see [Store Messages](/user-guide/rules-engine#store-messages).
...
- For a **JSON** view, use the Rules Engine's **Internal DB (JSON)** output node — see [Store as JSON](/user-guide/rules-engine#store-as-json) — or have your own device or service publish real JSON payloads to the channel.
