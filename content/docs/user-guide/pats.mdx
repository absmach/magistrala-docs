...
Available scope entities are **Clients**, **Channels**, **Groups**, and **Workspace**. Each entity type exposes a distinct set of operations:
...
<Callout type="warn" title="Operation names need verification">
  The operation identifiers below are carried over from the pre-rename docs. `auth/postgres/init.go`'s `auth_9` migration confirms `create_clients`/`list_clients` are stale and should be `create_devices`/`list_devices` (entity type `devices`, not `clients`) — this overlaps with the Clients→Devices rename tracked on a separate branch. Whether `list_domain_invitation` was similarly renamed (e.g. to `list_workspace_invitation`) could not be confirmed against the current source and needs a direct check before this table is trusted.
</Callout>
...
| `list_domain_invitation` | List all invitations for the workspace   |
