...
Magistrala IoT platform is comprised of core services (auth, users, groups, clients, channels, domains, and messaging services) together with additional services such as bootstrap, provision, consumers, readers, and the rules engine. For the full component reference, see the [Architecture page in the Dev Guide](/dev-guide/architecture).
